...
def save_voice_tactical(n, contents, filename, ba, subba, channel, site):
    ...
    return f"Saved voice_tactical_volume ({len(vol_df)}) and voice_tactical_aht ({len(aht_df)}) for scope {scope_key}. {dbg}", False
...
def save_bo_tactical(n, contents, filename, ba, subba, channel, site):
    ...
    return f"Saved bo_tactical_volume ({len(vol_df)}) and bo_tactical_sut ({len(sut_df)}) for scope {scope_key}. {dbg}", False
...
@callback(Output("dl-chat-forecast-tmpl","data"), Input("btn-dl-chat-forecast-tmpl","n_clicks"), prevent_initial_call=True)
...
@callback(Output("dl-chat-actual-tmpl","data"), Input("btn-dl-chat-actual-tmpl","n_clicks"), prevent_initial_call=True)
def dl_chat_ac_tmpl(_n):
    df = pd.DataFrame({"date":[], "items":[], "aht_sec":[]})
    return dcc.send_data_frame(df.to_csv, "chat_actual_template.csv", index=False)

@callback(Output("dl-ob-forecast-tmpl","data"), Input("btn-dl-ob-forecast-tmpl","n_clicks"), prevent_initial_call=True)
...
@callback(Output("dl-ob-actual-tmpl","data"), Input("btn-dl-ob-actual-tmpl","n_clicks"), prevent_initial_call=True)
...
def save_voice_forecast(_n, preview_rows, scope, ba, sba, ch, site):
    ...
    return f"Saved VOICE forecast ({len(df)}) for {sk}", False
...
def save_voice_actual(_n, preview_rows, scope, ba, sba, ch, site):
    ...
    return f"Saved VOICE actual ({len(df)}) for {sk}", False
...
def save_bo_forecast(_n, preview_rows, scope, ba, sba, ch, site):
    ...
    return f"Saved BO forecast ({len(df)}) for {sk}", False
...
def save_bo_actual(_n, preview_rows, scope, ba, sba, ch, site):
    ...
    return f"Saved BO actual ({len(df)}) for {sk}", False
...
def hc_preview(contents, filename):
    ...
    dff = df[cols].copy()
    return dff.to_dict("records"), pretty_columns(dff), f"Preview loaded: {len(df)} rows", None
...
@callback(Output("tbl-chat-forecast","data"), Output("tbl-chat-forecast","columns"), Output("chat-forecast-msg","children", allow_duplicate=True),
...
@callback(Output("tbl-chat-actual","data"), Output("tbl-chat-actual","columns"), Output("chat-actual-msg","children", allow_duplicate=True),
...
@callback(Output("chat-forecast-msg","children", allow_duplicate=True),
          Input("btn-save-chat-forecast","n_clicks"),
...
@callback(Output("chat-actual-msg","children", allow_duplicate=True),
          Input("btn-save-chat-actual","n_clicks"),
...
@callback(Output("tbl-ob-forecast","data"), Output("tbl-ob-forecast","columns"), Output("ob-forecast-msg","children", allow_duplicate=True),
...
@callback(Output("tbl-ob-actual","data"), Output("tbl-ob-actual","columns"), Output("ob-actual-msg","children", allow_duplicate=True),
...
@callback(Output("ob-forecast-msg","children", allow_duplicate=True),
          Input("btn-save-ob-forecast","n_clicks"),
...
@callback(Output("ob-actual-msg","children", allow_duplicate=True),
          Input("btn-save-ob-actual","n_clicks"),
...
@callback(Output("tbl-chat-forecast1","data"), Output("tbl-chat-forecast1","columns"), Output("chat-forecast-msg1","children", allow_duplicate=True),
...
@callback(Output("chat-forecast-msg1","children", allow_duplicate=True),
          Input("btn-save-chat-forecast1","n_clicks"),
...
@callback(Output("tbl-ob-forecast1","data"), Output("tbl-ob-forecast1","columns"), Output("ob-forecast-msg1","children", allow_duplicate=True),
...
@callback(Output("ob-forecast-msg1","children", allow_duplicate=True),
          Input("btn-save-ob-forecast1","n_clicks"),
